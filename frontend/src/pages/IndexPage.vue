...
const defaultIcon = L.icon({ iconUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-blue.png', shadowUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-shadow.png', iconSize:[25,41],iconAnchor:[12,41],popupAnchor:[1,-34],shadowSize:[41,41] })
const greenIcon   = L.icon({ iconUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-green.png', shadowUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-shadow.png', iconSize:[25,41],iconAnchor:[12,41],popupAnchor:[1,-34],shadowSize:[41,41] })
const redIcon     = L.icon({ iconUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-red.png',   shadowUrl:'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-shadow.png', iconSize:[25,41],iconAnchor:[12,41],popupAnchor:[1,-34],shadowSize:[41,41] })
const userIcon    = L.divIcon({ className:'', html:'<i class="fa-solid fa-person" style="color:#63E6BE;font-size:30px"></i>', iconSize:[24,24], iconAnchor:[12,12] })
const busIcon     = L.divIcon({ className:'', html:'<i class="fa-solid fa-bus" style="color:#86158e;font-size:30px"></i>',        iconSize:[24,24], iconAnchor:[12,12] })
...
export default {
  ...
  watch:{
    routes:{ handler(r){ this.routeGeometries=r.map(x=>x.points?x.points.slice():[]) }, deep:true, immediate:true },
    currentRoute:{ handler(r){ this.points=r&&r.points? r.points.slice():[] }, deep:true, immediate:true },
    recalcIdx(val){ if(val===this.selectedRouteIdx&&this.points.length>=2)this.calcRoute(val,this.points) },
    stops(newVal){ this.localStops=newVal.slice() }
  },
  computed:{
    routeLines(){ return this.routeGeometries.map((geom,idx)=>({geom,idx,route:this.routes[idx]})).filter(i=>i.route.visible&&i.geom.length>1) }
  },
  methods:{
    getMarkerIcon(i){
    ...
    onMapMouseMove(evt){
      const ll=evt.latlng
      if(this.stopsMode||this.stopsCleaning||this.cleaning) this.hoverCircle=[ll.lat,ll.lng]
      else if(this.editing&&this.points.length>=2&&this.isNearAnySegment(ll,50)) this.hoverCircle=[ll.lat,ll.lng]
